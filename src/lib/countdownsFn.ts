...
import { getAuthedDb } from "./server/db";
...
export const listCountdownsFn = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const rows = await db.select().from(countdowns).where(isNull(countdowns.archivedAt)).orderBy(countdowns.endsAt);
...
export const listArchivedCountdownsFn = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const rows = await db
    .select()
    .from(countdowns)
    .where(isNotNull(countdowns.archivedAt))
    .orderBy(desc(countdowns.archivedAt));
  return rows.map(fromRow);
});
...
export const createCountdownFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [row] = await db.insert(countdowns).values(toInsert(data, userId)).returning();
...
export const updateTagsFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await db.update(countdowns).set(patch).where(eq(countdowns.id, data.id));
  });
...
export const archiveCountdownFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { db } = await getAuthedDb();
    const now = Date.now();
    await db.update(countdowns).set({ archivedAt: now, updatedAt: now }).where(eq(countdowns.id, data.id));
  });
...
export const removeCountdownFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { db } = await getAuthedDb();
    await db.delete(countdowns).where(eq(countdowns.id, data.id));
  });
...
export const reconcileCountdownsFn = createServerFn({ method: "POST" }).handler(async () => {
  const { db } = await getAuthedDb();
  const now = Date.now();
  const rows = await db.select().from(countdowns);
